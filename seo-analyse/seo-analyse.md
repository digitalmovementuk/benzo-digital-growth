# Benzo × Digital Movement

## SEO & Website Proposal

- **Stand:** 13. Juli 2026
- **Unternehmen:** Benzo GmbH
- **Website:** [benzo-berlin.de](https://www.benzo-berlin.de/)
- **Markt:** Deutschland · Schwerpunkt Berlin
- **Zielgruppen:** Gastronomie · Hotellerie · Fachhandel
- **Projekt:** Website-Redesign + SEO · SEO AI+ optional

**Projektziel:** Eine neue Website und gezielte SEO-Seiten sollen mehr qualifizierte B2B-Anfragen gewinnen.

---

## Digital Growth für Benzo

### Mehr Leads im B2B-Markt

Eine neue Website.<br>
Gezielte SEO-Seiten für Gastronomie, Hotellerie und Fachhandel.<br>
Monatliche SEO-Optimierung und Performance-Reports.<br>
SEO AI+ ist als separates Add-on verfügbar.

---

## Inhalt

1. Executive Summary
2. Benzo SEO-Analyse
3. Neue SEO-Seiten
4. Projektablauf
5. Erwartete SEO-Entwicklung
6. SEO AI+
7. Website-Vorschau
8. Pakete & Preise
9. Ihr Projektteam
10. Digital Movement

---

## 01 — Executive Summary

### So gewinnt Benzo mit SEO neue B2B-Kunden

Benzo hat bereits Markenvertrauen und erste Rankings ohne Markenbegriff.<br>
Der nächste Schritt sind eigene SEO-Seiten für konkrete Suchanfragen von Gastronomie, Hotellerie und Fachhandel.

1. **Gefunden werden:** Neukunden suchen ohne „Benzo“, zum Beispiel nach „italienische Lebensmittel Großhandel“.
2. **Passende Seite:** Jede klare Suchabsicht erhält eine eigene SEO-Seite mit Sortiment, Liefergebiet, B2B-Vorteilen und Kontaktweg.
3. **Anfrage gewinnen:** Die neue Website führt Besucher über klare Nachweise und Kontaktmöglichkeiten zur B2B-Anfrage.

| Priorisierter Suchbegriff | Suchvolumen | Neue SEO-Seite | Bestehende Position der Startseite |
|---|---:|---|---:|
| italienische Lebensmittel Großhandel | 480/Monat + verwandter Begriff 210/Monat | `/italienische-lebensmittel-grosshandel/` | 36 |
| italienischer Großhandel Berlin | 140/Monat | `/italienischer-grosshandel-berlin/` | 5 |

Weitere bestehende Ranking-Chancen sind „italienische Lebensmittel Berlin“ auf Position 9, „italienische Feinkost Berlin“ auf Position 10 und „italienischer Supermarkt Berlin“ auf Position 11.<br>
Die dazugehörigen Seitendetails werden mit den übrigen geschützten Empfehlungen im SEO Page Guide freigegeben.

**Empfohlene Umsetzung:** 20 neue SEO-Seiten, sechs Monate Optimierung und Reporting sowie eine Website, die für B2B-Anfragen entwickelt wird.<br>
Die genannten Positionen gehören heute der bestehenden Startseite.<br>
Die neuen SEO-Seiten existieren noch nicht.<br>
Die Seiten werden monatlich ausgewertet und anhand ihrer tatsächlichen Leistung optimiert.

---

## 02 — Benzo SEO-Analyse

### Benzo ist als Marke etabliert.<br>
*Es fehlen Reichweite und eigene SEO-Seiten.*

Die bestehende Website wird vor allem über Suchanfragen mit dem Namen „Benzo“ gefunden.<br>
Die XML-Sitemap der Hauptdomain führt nur vier URLs auf.<br>
Eigene Seiten für Sortimente, Leistungen und B2B-Suchanfragen fehlen.

### Der zentrale Befund

**Ca. 97 % markenbezogener Traffic.**<br>
**Ca. 3 % allgemeiner Traffic.**

Semrush schätzt 486 organische Besuche pro Monat.<br>
Diese Werte beschreiben geschätzten Suchtraffic, nicht Leads.

| Kennzahl | Eindeutige Einordnung |
|---|---|
| 75 organische Keywords | Alle Positionen, inklusive Markenbegriffen |
| 4 URLs | In der XML-Sitemap aufgeführt; nicht als Indexierungszahl zu verstehen |
| 3 H1 | Auf der Startseite; keine strukturierten Daten erkannt |

### Bestehende Rankings der Startseite

Die folgende Auswahl zeigt vier Suchbegriffe ohne „Benzo“, für die heute die **bestehende Startseite** rankt.<br>
Eine eigene SEO-Seite existiert für diese Themen noch nicht.

| Suchbegriff | Position der bestehenden Startseite | Suchen pro Monat |
|---|---:|---:|
| italienischer Großhandel Berlin | 5 | 140 |
| italienische Lebensmittel Berlin | 9 | 210 |
| italienische Feinkost Berlin | 10 | 210 |
| italienischer Supermarkt Berlin | 11 | 1.000 |

**Ist-Zustand:** Die Startseite rankt.<br>
**Im Projekt:** Eine neue eigene SEO-Seite wird für das Thema erstellt.<br>
**Zielrichtung:** Das Ranking soll durch höhere Relevanz und laufende Optimierung verbessert werden.

> **Warum erscheint derselbe Suchbegriff in Abschnitt 03 erneut?**<br>
> Hier steht das heutige Ranking der Startseite.<br>
> In Abschnitt 03 steht die neue SEO-Seite, die heute noch nicht existiert.

### Datengrundlage

Analyse erstellt am **13.07.2026**.<br>
Semrush-Datenstand **12.07.2026**.

![Semrush Domain Overview für benzo-berlin.de](assets/semrush-domain-overview-benzo.png)

![Semrush Organic Rankings für benzo-berlin.de](assets/semrush-organic-rankings-benzo.png)

### Wettbewerbsanalyse

| Domain | Organische Keywords | Geschätzte organische Besuche/Monat | Einordnung |
|---|---:|---:|---|
| **benzo-berlin.de** | **75** | **486** | Ausgangspunkt |
| fresco-berlin.de | 248 | 461 | Direkter Wettbewerber nach Region, Zielgruppe und Angebot |
| asimex.de | 91 | 897 | Vergleich für B2B-Struktur und Bestellprozess |
| italia-import.de | 2.972 | 10.334 | Tiefe Kategorien, Shop und Großkundenbereich |
| bell-italia.com | 2.447 | 9.726 | Große Marken- und Produktabdeckung |
| centro-italia.de | 12.164 | 61.750 | Benchmark für Kategorie- und Inhaltstiefe |

#### Wettbewerber auf Seite 1 · Benzo noch nicht

| Suchbegriff | Benzo | Wettbewerber auf Seite 1 |
|---|---:|---|
| italienischer Großhandel | Position 18 | Italia-Import, Bell Italia, ASIMEX, Fresco Berlin |
| italienische Lebensmittel Großhandel | Position 36 | Italia-Import, Bell Italia, ASIMEX |
| italienische Feinkost online | Nicht Top 10 | Centro Italia, Gustini, Viani, Italia-Import |
| italienische Lebensmittel online | Nicht Top 10 | Centro Italia, Gustini, Italia-Import |

**Wichtig:** „italienischer Großhandel Berlin“ und „italienischer Großhandel“ sind zwei verschiedene Suchanfragen.<br>
Der lokale Begriff rankt auf Position 5.<br>
Der breitere Begriff rankt auf Position 18.

Benzo braucht eigene SEO-Seiten für Suchbegriffe, die Wettbewerber bereits auf Seite 1 besetzen.

---

## 03 — Neue SEO-Seiten

### 20 neue SEO-Seiten

Das reguläre Growth-Paket umfasst **7 datenbasierte Kernseiten** und **8 ergänzende Growth-Seiten**.<br>
Die Benzo-Sonderkondition ergänzt **5 weitere Seiten**.<br>
Alle 20 Seiten werden neu erstellt und veröffentlicht.

Das kumulierte monatliche Suchvolumen der geprüften Begriffe beträgt 2.940.<br>
Die Entwicklung der neuen Seiten wird im monatlichen Reporting dokumentiert.

> **2 von 20 Empfehlungen sind vollständig sichtbar.**<br>
> Die weiteren 18 SEO-Seiten werden nach Projektstart im SEO Page Guide freigegeben.

| Empfohlene neue SEO-Seite | Suchbegriffe · Volumen | Bestehender Zustand | Priorität |
|---|---|---|---|
| Italienische Lebensmittel Großhandel · `/italienische-lebensmittel-grosshandel/` | `italienische lebensmittel großhandel` · 480/Monat<br>`großhandel italienische lebensmittel` · 210/Monat | Neue SEO-Seite existiert nicht.<br>Startseite: Position 36 | A · B2B-Kernseite |
| Italienischer Großhandel Berlin · `/italienischer-grosshandel-berlin/` | `italienischer großhandel berlin` · 140/Monat | Neue SEO-Seite existiert nicht.<br>Startseite: Position 5 | A · lokaler B2B-Quick-Win |
| █████████████████ | 1.000/Monat | Freigabe nach Projektstart | B · lokale Store-Reichweite |
| █████████████████ | 210 + 170/Monat | Freigabe nach Projektstart | A · Quick Win |
| █████████████████ | 210 + 170 + 90/Monat | Freigabe nach Projektstart | A · Quick Win |
| █████████████████ | 170/Monat | Freigabe nach Projektstart | B · Aufbau |
| █████████████████ | 90/Monat | Freigabe nach Projektstart | B · Zusatzmarkt |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · Zielgruppe |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · Zielgruppe |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · Sortiment |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · Service |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · B2B |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · Sortiment |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · Sortiment |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · Sortiment |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · B2B |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · Sortiment |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · Service |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · Liefergebiet |
| █████████████████ | Im SEO Page Guide | Freigabe nach Projektstart | B · Zusatzmarkt |

Semrush Keyword Overview, Deutschland, Desktop; abgerufen am 13.07.2026.<br>
Die angegebenen Volumen entsprechen dem monatlichen Suchvolumen gemäß Semrush.

---

## 04 — Projektablauf

Die SEO-Erst-Analyse ist bereits geliefert.<br>
Nach Paketauswahl und beidseitiger Vertragsunterschrift beginnt die Umsetzung.<br>
Die erste Rechnung stellen wir erst beim Launch.

> **Welcher Ablauf gilt?**<br>
> Mit Website-Modul entsteht das Website-Redesign gemeinsam mit den neuen SEO-Seiten.<br>
> Ohne Website-Modul integrieren wir die SEO-Seiten in die bestehende Website.<br>
> Die Benzo-Empfehlung in Abschnitt 08 umfasst Website und SEO.<br>
> SEO AI+ kann separat hinzugebucht werden.

1. **Erst-Gespräch · abgeschlossen:** Ausgangslage, Ziele und Projektumfang wurden aufgenommen.
2. **SEO-Erst-Analyse · abgeschlossen:** SEO-Status, Potenzial, Wettbewerb und Vorschläge für neue SEO-Seiten wurden analysiert.
3. **Projektvorschlag · aktueller Schritt:** Strategie, Seitenumfang, Ablauf und Konditionen werden abgestimmt.
4. **Vertragsunterschrift:** Benzo bestätigt Paket, Leistungsumfang und sechsmonatige SEO-Laufzeit.
5. **SEO-Strategie-Kick-off-Workshop · Woche 1:** Zielgruppen, Märkte, Leistungen, Produkte, Liefergebiete und Messgrößen werden definiert.
6. **SEO Page Guide · Woche 1–2:** Liste, Aufgabe und Reihenfolge der neuen SEO-Seiten. **Freigabe durch Benzo.**
7. **Website-Redesign & Entwicklung · Woche 2–4 · mit Website-Modul:** UX/UI, Navigation, Seitentypen, Kontaktwege und responsive Entwicklung. **Freigabe durch Benzo.**
8. **Texte für die SEO-Seiten · Woche 3–6:** Die freigegebenen Seiten werden geschrieben und umgesetzt. **Freigabe durch Benzo.**
9. **Launch und Google-Einreichung:** Alle gebuchten SEO-Seiten gehen live.<br>Das Website-Modul geht gleichzeitig live, sofern es beauftragt wurde.<br>Beim Launch werden die einmalige Website-Rechnung und die erste von sechs SEO-Monatsrechnungen gestellt.
10. **Monate 1–6 · enthalten:** Monatliche Optimierung und Reporting.<br>Bei separater Buchung von SEO AI+ fließen zusätzlich Markennennungen, Zitationen, Wettbewerber und AI-Sichtbarkeit ein.<br>Die sechsmonatige SEO-Laufzeit beginnt mit dem Launch.
11. **Ab Monat 7 · nicht enthalten:** Eine Fortführung benötigt eine separate schriftliche Vereinbarung.

**Zeitplan-Annahme:** Woche 1 beginnt mit dem Kick-off.<br>
Der Zeitplan setzt Benzo-Freigaben innerhalb von zwei Werktagen voraus.

---

## 05 — Erwartete SEO-Entwicklung

Die Kurve zeigt die angestrebte qualitative Entwicklungsrichtung.<br>
Die angebotene SEO-Laufzeit umfasst die Monate 1–6.

| Phase | Zeitraum | Vertragsstatus | Mögliche Entwicklung |
|---|---|---|---|
| Fundament | Monate 1–3 | Enthalten | Google verarbeitet Struktur, Technik und neue SEO-Seiten. |
| Wachstum | Monate 4–6 | Enthalten | Erste neue Rankings können entstehen. |
| Ausbau | Monate 7–9 | Nur bei schriftlicher Verlängerung | Erfolgreiche Seiten können weiter optimiert werden. |
| Stabilisierung | Monate 10–12+ | Nur bei schriftlicher Verlängerung | Starke Seiten können weiter gefestigt werden. |

Rankings, Sichtbarkeit und Seitenleistung werden monatlich ausgewertet und priorisiert optimiert.

---

## 06 — SEO AI+

### Sichtbarkeit in Google und in KI-Antworten

Benzo soll überall sichtbar sein, wo potenzielle Kunden nach italienischen Lieferanten, Sortimenten oder Einkaufsmöglichkeiten suchen — bei Google, ChatGPT, Gemini und Perplexity.<br>
SEO AI+ misst diese Sichtbarkeit wöchentlich und übersetzt die Ergebnisse in konkrete Optimierungsmaßnahmen.

| Leistungsbereich | Was Benzo erhält | Ergebnis |
|---|---|---|
| Wöchentliche AI-Analyse | Prüfung von ChatGPT, Gemini und Perplexity mit dokumentierter Veränderung zur Vorwoche | Entwicklung wird sichtbar |
| Markenfragen und Erwähnungen | Bis zu fünf individuelle Fragen zu Benzo, Sortiment und Einkaufssituation | Nennungen und Positionen werden messbar |
| Wettbewerbervergleich | Vergleich von Markennennungen, Positionen und Bewegungen | Sichtbarkeitslücken werden priorisiert |
| Quellen- und Zitationsanalyse | Auswertung zitierter URLs und Domains | Quellenchancen werden erkennbar |
| Markenstimmung | Auswertung positiver und kritischer Signale im Wettbewerbsvergleich | Wahrnehmung wird verständlich |
| AI- und Shop-Fundament | Strukturierte Produktdaten, Produkt-Sitemaps, Canonicals, interne Links sowie optimierte Titel und Beschreibungen | Inhalte werden für Such- und KI-Systeme klarer lesbar |

Der AI-Sichtbarkeitsindex fasst Markennennungen, Positionen, Zitationen und Tonalität auf einer Skala von 0 bis 100 zusammen.<br>
Die Erkenntnisse fließen laufend in Inhalte, interne Verlinkung, Produktdaten und technische Signale ein.

---

## 07 — Website-Vorschau

[Interaktive Benzo Website-Vorschau öffnen](https://digitalmovementuk.github.io/benzo-digital-growth/)

Der Entwurf zeigt die visuelle Richtung und das responsive Verhalten.<br>
Er ist noch keine finale Website.<br>
Navigation, Texte und Kontaktwege werden innerhalb des gebuchten Leistungsumfangs finalisiert.

---

## 08 — Pakete & Preise

### Drei SEO-Pakete

| SEO-Paket | Neue SEO-Seiten | Preis pro Monat | SEO-Gesamtpreis · 6 Monate |
|---|---:|---:|---:|
| **Basic** | 5 | **1.199 EUR** | **7.194 EUR** |
| **Growth** | 15 | **2.498 EUR** | **14.988 EUR** |
| **Max** | 50 | **4.898 EUR** | **29.388 EUR** |

Jedes SEO-Paket enthält SEO-Konzeption, Text, Seitenlayout, technische Umsetzung, Google-Einreichung, monatliche Optimierung und Reporting.<br>
Jedes SEO-Paket hat sechs Monate Mindestvertragslaufzeit.

**Die SEO-Pakete sind eigenständig buchbar.**<br>
Ohne Website-Modul integrieren wir die neuen SEO-Seiten in die bestehende Website.<br>
Ein vollständiges Website-Redesign ist nicht im regulären SEO-Paketpreis enthalten.

### Separates Website-Modul

**High-End Website-Redesign & Entwicklung: 11.999 EUR einmalig**

Enthalten sind individuelles UX/UI-Design, responsive Entwicklung, Konversionsoptimierung, Inhaltsintegration, technisches SEO-Fundament und Launch.<br>
Der bestehende Shop wird verlinkt.<br>
Nicht enthalten sind Shop-Neubau, Produktmigration, Fotografie, Hosting, Domain, kostenpflichtige Drittanbieter-Lizenzen und komplexe individuelle Integrationen.

### Separates Add-on: SEO AI+

**Regulärer Retainer: 799 EUR pro Monat**

Enthalten sind wöchentliche Sichtbarkeitsanalysen in ChatGPT, Gemini und Perplexity, individuelle Markenfragen, Wettbewerbsvergleich, Quellen- und Zitationsanalyse, Markenstimmung sowie priorisierte inhaltliche und technische Optimierungen.

### Unsere Empfehlung für Benzo

**Growth · Benzo-Sonderkondition**

> **+5 EXTRA SEO-SEITEN INKLUSIVE**  
> Regulär 15 Seiten → Benzo 20 Seiten

- Regulärer SEO-Preis: 2.498 EUR/Monat
- Benzo-Preis: **1.249 EUR/Monat für sechs Monate**
- Rabatt auf den monatlichen SEO-Preis: **50 %**
- Enthalten: **20 neue SEO-Seiten** plus monatliche Optimierung und Reporting
- Benzo-Vorteil: **5 zusätzliche Seiten** gegenüber dem regulären Growth-Paket
- Website-Modul regulär: 11.999 EUR
- Website-Modul mit Sonderkondition: **299 EUR einmalig**
- Leistungsumfang des Website-Moduls: identisch; nur der Preis ist reduziert
- Sechs Monate SEO: 7.494 EUR
- **Gesamtpreis für sechs Monate SEO + Website-Modul: 7.793 EUR**

> **Frist:** Die Sonderkondition gilt, wenn beide Parteien den Vertrag spätestens am Freitag, 31. Juli 2026 unterschrieben haben.

Beim Launch werden 299 EUR für das Website-Modul und die erste SEO-Rate von 1.249 EUR berechnet.<br>
Fünf weitere SEO-Raten folgen monatlich.<br>
SEO AI+ ist ein separates Add-on und nicht im Benzo-Gesamtpreis enthalten.<br>
Monate 7–12 sind nicht enthalten und erfordern eine separate schriftliche Vereinbarung.<br>
Alle Preise sind Nettopreise.<br>
Umsatzsteuer kommt nur hinzu, soweit sie gesetzlich geschuldet ist.

---

## 09 — Ihr Projektteam

### Projektteam

- **Martey Quaye — Co-Founder & Digital Strategy:** Strategische Ausrichtung und digitale Wachstumsarchitektur. [LinkedIn](https://www.linkedin.com/in/martey-quaye-64a05017b/)
- **Raoul (Alex) Müller — Co-Founder & Account Manager:** Strategie, Freigaben, Umsetzung und Reporting. [LinkedIn](https://www.linkedin.com/in/raoulmueller)
- **Fabian Pianka — SEO Copy Writer Germany:** Deutschsprachige SEO-Inhalte passend zu Benzo, Zielgruppen und Sortiment.
- **Dean Bosilkovski — SEO Operations Manager:** SEO-Struktur, technische Qualität, Umsetzung und laufende Optimierung. [LinkedIn](https://www.linkedin.com/in/deanbosilkovski/)

### Ihr Account Manager

**Raoul (Alex) Müller**

- WhatsApp: [+49 176 23296439](https://wa.me/4917623296439)
- Telefon: [+49 176 82360647](tel:+4917682360647)
- E-Mail: [alex@digitalmovement.uk](mailto:alex@digitalmovement.uk)

Nach Paketauswahl und beidseitiger Vertragsunterschrift vereinbaren wir den Kick-off.<br>
Dann legen wir die Reihenfolge der neuen SEO-Seiten fest.

---

## 10 — Digital Movement

Seit 2018 helfen wir Unternehmen, online sichtbar zu werden, Kundenanfragen zu generieren und Umsätze zu steigern.

| Kennzahl | Digital Movement |
|---|---:|
| Gegründet in Melbourne, AU | 2018 |
| Betreute Kunden | 300+ |
| 5-Sterne-Google-Bewertungen | 100+ |
| Kundenbindungsrate | 95 % |

### Kernleistungen

- SEO
- GEO / AI Search Optimisation
- Website Development
- Ads
- CRM
- Automation & AI

[Alle Google Reviews von Digital Movement Melbourne ansehen](https://www.google.com/search?q=Digital+Movement+Melbourne+Reviews)

[Testimonials, Success Stories, Reviews und Case Studies ansehen](https://success.digitalmovement.uk/)

---

## Digital Movement Offices

**Driving *Digital Growth* across the globe.**

### London Office · United Kingdom

- 128 City Road, London, EC1V 2NX
- [+44 (0) 207 689 7888](tel:+442076897888)
- [office@digitalmovement.uk](mailto:office@digitalmovement.uk)
- [digitalmovement.uk](https://digitalmovement.uk)

### Auckland Office · New Zealand

- Level 8, 139 Quay Street, Auckland 1010
- [+64 (0) 9 363 2929](tel:+6493632929)
- [office@digitalmovement.co.nz](mailto:office@digitalmovement.co.nz)
- [digitalmovement.co.nz](https://www.digitalmovement.co.nz)

### Berlin Office · Germany

- Potsdamer Platz 1, 10117 Berlin
- [+49 (0) 176 82360647](tel:+4917682360647)

### Melbourne HQ · Australia

- Level 3/480 Collins St, Melbourne VIC 3000
- [+61 (0) 300 988 161](tel:+61300988161)
- [office@digitalmovement.com.au](mailto:office@digitalmovement.com.au)
- [digitalmovement.com.au](https://www.digitalmovement.com.au)

---

## Quellen und Datenhinweis

- Semrush Deutschland: Domain Overview, Organic Research, Competitor Research und Keyword Research
- Google-Suchergebnisse für die geprüften Suchbegriffe
- Öffentliche Website- und Technikprüfung von Benzo und den genannten Wettbewerbern
- Digital Movement Success Stories, Testimonials und Google Reviews auf [success.digitalmovement.uk](https://success.digitalmovement.uk/)

Analyse erstellt am 13.07.2026.<br>
Semrush-Datenstand 12.07.2026.<br>
Die weitere Entwicklung wird im monatlichen Reporting dokumentiert.
